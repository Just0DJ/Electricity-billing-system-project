...
''')
    ...
    c1.execute("delete from newcustomer where accountno='{}'".format(acc))
    c1.execute("delete from Signup where username='{}'".format(use))
    con.commit()
    delay('''
╔═══════════════════════════════════════════════════════════════╦═══╗
...
""")
        ...
        if choice == "1":
            ...
            # Units vs Amount
            c1.execute("select Todays, unitsconsumed, amtwithvat from Transaction where accountno=%s order by Todays", (accountno,))
            data = c1.fetchall()
            if data:
                dates = [row[0] for row in data]
                ...
                plt.bar([i+0.4 for i in x], amounts, width=0.4, label="Amount (₹)")
            ...
            c1.execute("select status, count(*) from Transaction where accountno=%s group by status", (accountno,))
            data = c1.fetchall()
            if data:
