...
""")
            ...
            accountno = int(input('Enter your account number for graph: '))
            graph(accountno)
            _Intro(data)
        elif choice2==5:
            delay("""
...
''')
    ...
    if _pass==confirmpass:
        c1.execute("insert into signup values(%s,%s,%s)", (username, _pass, confirmpass))
        VAR_1=input("Do you want to continue?(yes/no): ")
    else:
        delay("""
╔═══════════════════════════════════════════════════════════════╦═══╗
...
''')
    ...
    info02="select * from signup where username=%s and _pass=%s"
    c1.execute(info02, (username, _pass))
    data=c1.fetchall()
    if len(data)==0:
        delay("""
╔═══════════════════════════════════════════════════════════════╦═══╗
...
''')
    ...
    print(" \n")
    info2="insert into newcustomer values(%s,%s,%s,%s,%s)"
    c1.execute(info2, (accountno, bankname, name, address, phoneno))
    con.commit()
...
''')
    ...
    c1.execute("delete from Transaction where accountno=%s", (acc,))
    c1.execute("delete from newcustomer where accountno=%s", (acc,))
    c1.execute("delete from Signup where username=%s", (use,))
    con.commit()
    delay('''
╔═══════════════════════════════════════════════════════════════╦═══╗
...
''')
    ...
    info10="select * from newcustomer where accountno=%s"
    c1.execute(info10, (accountno,))
    data3=c1.fetchall()
    VAR_01="select * from transaction where accountno=%s"
    c1.execute(VAR_01, (accountno,))
    VAR_02=c1.fetchall()
    print(data3)
    if len(data3)==0:
        delay('''
...
""")              
            ...
            info3="insert into Transaction values(%s,%s,%s,%s,%s,%s,%s)"
            c1.execute(info3, (accountno, unit, Todays, amount, VAT, totalamt, paid))
            con.commit()
            VAR_2=input("Do you want to continue?(yes/no): ")
            if VAR_2=="yes":
                print()
...
""")
    accountno=int(input('Enter your account number: '))
    info4="select * from newcustomer where accountno = %s"
    c1.execute(info4, (accountno,))
    data1=c1.fetchall()
    if len(data1)==0:
        delay("""
╔═══════════════════════════════════════════════════════════════╦═══╗
...
""")
        ...
        for row in data1:
            ...
            print("\n")
        info5="select * from Transaction where accountno = %s"
        c1.execute(info5, (accountno,))
        data2=c1.fetchall()
        for row in data2:
            print("Unit: ",row[1])
            print("paid on: ",row[2])
...
╠═══════════════════════════════════════════════════════════════╩═══╣
║                                                                   ║
║ 1.Bill Amount Trend (Line Graph)                                  ║
║ 2.Units Consumed vs Amount Paid (Bar Chart)                       ║
...
""")
        ...
        if choice == "1":
            # Bill Amount Over Time
            c1.execute("select Todays, amtwithvat from Transaction where accountno=%s order by Todays", (accountno,))
            data = c1.fetchall()
            if data:
                dates = [row[0] for row in data]
                bills = [row[1] for row in data]
                plt.plot(dates, bills, marker="o", linestyle="-", color="blue")
                plt.title(f"Bill Amount Trend for Account {accountno}")
                plt.xlabel("Date")
            ...
            # Units vs Amount
            c1.execute("select Todays, unit, amtwithvat from Transaction where accountno=%s order by Todays", (accountno,))
            data = c1.fetchall()
            if data:
                dates = [row[0] for row in data]
                ...
                plt.bar(x, units, width=0.4, label="Units Consumed")
                plt.bar([i+0.4 for i in x], amounts, width=0.4, label="Amount (AED)")
                plt.xticks([i+0.2 for i in x], dates, rotation=45)
                plt.title(f"Units vs Amount for Account {accountno}")
                plt.legend()
                plt.show()
            else:
                print("No transactions found.")
            ...
            c1.execute("select PAID, count(*) from Transaction where accountno=%s group by PAID", (accountno,))
            data = c1.fetchall()
            if data:
                labels = [row[0] for row in data]
                values = [row[1] for row in data]
                plt.pie(values, labels=labels, autopct="%1.1f%%", startangle=140)
                plt.title(f"Payment Status for Account {accountno}")
                plt.show()
            else:
                print("No transactions found.")
